...
if ($streamUrldecoded != "" && $streamTitledecoded != "") {
	...
	if (useAPI()) {
		...
		$query = "SELECT COUNT(*) FROM streams_comments WHERE url = '$streamQuery' AND liked = '1' AND user = '".$_SESSION['escapedemail']."' AND siteID = '".$_SESSION['siteID']."'";
	...
	if ($num_rows_like_count == 0) {
		...
		if (useAPI()) {
			...
			$stmt = $db->stmt_init();
			$sql = "INSERT INTO streams_comments (url, title, image, user, liked, excerpt, siteID, comment) VALUES (?, ?, ?, ?, '1', ?, ?, '');";
			$stmt->prepare($sql);
			$stmt->bind_param("sssssi", $streamUrldecoded, $streamTitledecoded, $trimmedimageurl, $userposter, $streamExcerpt, $_SESSION['siteID']);
			$stmt->execute();
			$stmt->close();
			$db->close();
		}
		...
		if (useAPI()) {
			...
			$stmt = $db->stmt_init();
			$sql = "DELETE FROM streams_comments WHERE url = ? AND liked = '1' AND user = ? AND siteID = ?";
			$stmt->prepare($sql);
			$stmt->bind_param("ssi", $streamUrldecoded, $_SESSION['useremail'], $_SESSION['siteID']);
			$stmt->execute();
			$stmt->close();
			$db->close();
		}
