...
require(dirname(__FILE__) . '/../../core/abre_dbconnect.php');
try {
	$sql = "SELECT title, image, url, creationtime, excerpt 
					FROM streams_comments 
					WHERE user = '$_SESSION[escapedemail]' AND liked = 1 AND siteID = $_SESSION[siteID]
	...
	while (!empty($posts)) {
		...
		if ($title != "" && $excerpt != "") {
			displayUneditablePostWithMessage($type, $id, $title, $image, $date, $rawExcerpt, $excerpt, $rawLink,
				$likeCount, $userLiked, $commentCount, $cardCount, "You Liked This");
			$cardCount++;
		}
	}
} finally {
	...
	<div class="row center-align">
		<div class="widget" style="padding:30px; text-align:center; width:100%;">
			...
			<p class="default-message-description">
				Get started by liking a post from your stream.
				<br>
				You'll see the stream posts you have liked here.
			</p>
		</div>
	</div>
	<?php
